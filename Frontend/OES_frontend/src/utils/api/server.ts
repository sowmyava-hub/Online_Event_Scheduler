import { Poll, Vote, HttpResponse, Time, Recevent } from "../../models/poll";
...
const getMycalendar = (
  ...
  const endpoint = `${NEXT_PUBLIC_BASE_URL}/api/meeting/${userid}`;
  const requestOptions: RequestInit = {
    method: "GET",
  };
  return httpMethod(endpoint, requestOptions);
...
const markmeetingTimes = (voteArgs: {
  newVote: Recevent;
  pollID: string;
}): Promise<HttpResponse> => {
  const { newVote, pollID } = voteArgs;
  const endpoint = `${NEXT_PUBLIC_BASE_URL}/api/meeting/${pollID}`;
  const requestOptions: RequestInit = {
    method: "PUT",
    body: JSON.stringify(newVote),
  };
...
export { getPoll, createPoll, markTimes, markmeetingTimes, markFinalTime, deletePoll, getMycalendar };
