...
export  default function Display() {
  ...
  //Api endpoint to get Data block
  const [data, setData] = useState(null);

  useEffect(() => {
    async function fetchData() {
        const response = await axios.get('http://34.125.236.71:8800/get_poll?poll_id=30');
        response.data.from_slot=response.data.from_slot.split(" ")[1];
        response.data.to_slot=response.data.to_slot.split(" ")[1];
      ...
      <Row className="create-event__data">
        ...
        {data && (
          <Form>
            ...
            <h2>Initiator Name (Organizer)</h2>
            <Form.Group controlId="safeWord">
              <Form.Control disabled
                name="safeWord"
                type="text"
                placeholder="Initiator Name (Organizer)"
        ...
        >
          <Form.Group controlId="isRecurrent" style={{paddingLeft:100}}>
            <h2><b>Availability</b></h2>
              <Form.Check
                name="isRecurrent"
                type="switch"
                label="Do you want to join the meeting?(Yes/No)?"
